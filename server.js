...
function getNetworkOrigin(req) {
  const host = String(req.headers.host || "").split(":")[0];
  const port = String(req.headers.host || "").split(":")[1] || PORT;
...
function getJoinUrl(req, gameCode) {
  return `${getNetworkOrigin(req)}/j/${encodeURIComponent(gameCode)}`;
}
...
function handleCreateRoom(req, res, body) {
  ...
  sendJson(res, 201, {
    gameCode: room.gameCode,
    joinUrl: getJoinUrl(req, room.gameCode),
    room: getRoomState(room),
  });
}
...
function handleNext(res, body) {
  const room = ensureRoom(body.gameCode);
  if (room.state !== "playing" && room.state !== "revealing") {
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && pathname === "/config") {
      ...
      sendJson(res, 200, {
        joinUrl: gameCode ? getJoinUrl(req, gameCode) : null,
        networkOrigin: getNetworkOrigin(req),
      });
      return;
    }
